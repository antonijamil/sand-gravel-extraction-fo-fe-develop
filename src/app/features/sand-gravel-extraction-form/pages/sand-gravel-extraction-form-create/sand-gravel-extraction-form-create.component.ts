...
export class SandGravelExtractionFormCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      //captain: new FormControl({value: '', disabled: true}, Validators.required),
  ...
  addNewRegistryEntry() {
    ...
    if (this.form.valid) {
      ...
      registerEntry.user = this.currentUser;
      this.registerEntriesService.add(registerEntry).subscribe(res => {

        if (!res) {

          this.messageService.add({severity: 'error', summary: 'Erreur serveur', detail: ''});

        } else {
          this.messageService.add({severity: 'success', summary: 'Formulaire sauvegardé!', detail: ''});
        }
      });
